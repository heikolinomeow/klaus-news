...
usage() {
  cat <<'EOF'
Usage:
  scripts/claude/masteragent-runner.sh [--retries N] [--leaf-progress] [--verbose]
...
: "${CMD_NB_STAGE:=bash scripts/claude/nb-leaf-runner.sh --retries ${RETRIES} ${leaf_progress_flag}}"
: "${CMD_BTS_STAGE:=bash scripts/claude/bts-leaf-runner.sh --retries ${RETRIES} ${leaf_progress_flag}}"
: "${CMD_STCC_STAGE:=bash scripts/claude/stcc-stage-runner.sh --retries ${RETRIES} ${leaf_progress_flag}}"
: "${CMD_IC_STAGE:=bash scripts/claude/ic-leaf-runner.sh --retries ${RETRIES} ${leaf_progress_flag}}"

LOG_DIR="${LOG_DIR:-.tmp/masteragent-claude}"
mkdir -p "$LOG_DIR"
