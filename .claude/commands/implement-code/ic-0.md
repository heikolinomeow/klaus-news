---
description: Code Patches Implementer (Confirmed → Repo)
---
...
## Mission
Read `docs/code_patches_confirmed.md` and implement the described patch operations into the repo, step by step.
...
- `docs/code_implementation.md` (the full step-by-step run log)
...
You may write ONLY:
1) Repo files explicitly referenced by ops in `docs/code_patches_confirmed.md`
2) `docs/code_implementation.md`
...
## OP block requirements (must be present in `docs/code_patches_confirmed.md`)
...
- **INSERT BEFORE/AFTER** idempotent if:
  ...
  - it is immediately before/after the anchor occurrence as specified
...
### Step 1 — Parse plan
1) Read `docs/code_patches_confirmed.md`
...
## Output: `docs/code_implementation.md` (required structure)
