...
Klaus News v2.0 addresses three critical pillars:
...
Current system has critical data loss risks: a single `docker-compose down -v` command wipes all configuration (lists, settings, post history) with no recovery mechanism. Key logic components (worthiness scoring, duplicate detection) use hardcoded algorithms that cannot be adjusted without code changes.
...
### 2.1 Database Brittleness (Critical)
...
### 2.2 Configuration Lock-In (High)
...
### 2.4 Algorithmic Rigidity (Medium)
...
### 2.5 Operational Inflexibility (Medium)
...
### 3.1 System Administrator (primary)
...
**UI Toggle (Settings → Scheduling tab)**:
...
#### 4.4.3 Current Prompts (from PROMPTS_REFERENCE.md)
...
**5. Worthiness Scoring** (new prompt, see Feature 4.6)
**6. Duplicate Detection** (new prompt, see Feature 4.7)
...
**Risk**: 2/10 - Low risk, reuses patterns from Feature 4.2. Main risk is ensuring prompt validation prevents broken configurations.
...
**New AI Prompt: Worthiness Scoring**:
...
**Integration into Ingestion Flow**:
...
**New AI Prompt: Duplicate Detection**:
...
### 5.1 Phase 1: Foundation (Data Safety)
...
### 5.2 Phase 2: Configuration Transparency
...
### 5.3 Phase 3: AI-First Architecture
...
### 5.4 Phase 4: Monitoring & Optimization
...
### 7.2 Configuration Portability Metrics
...
## 9. Migration Path from v1.1 to v2.0

### 9.1 Step 1: Backup Current State
...
## 11. Appendix: Current System Architecture

### 11.1 AI Prompt Execution Points (from PROMPTS_REFERENCE.md)
